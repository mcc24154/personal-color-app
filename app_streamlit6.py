...
# 'color_analyzer' モジュールは実行環境にあると仮定
...
def set_cosmetic_flow_css():
    st.markdown(
        ...
        .cosmetic-flow {
            white-space: nowrap; 
            animation: flow-right-to-left 40s linear infinite; 
            padding-top: 10px;
        }
...
def show_start_page():
    if not bg_base64 or not logo_base64 or not deco_base64:
        st.error("⚠️ 画像データが空です。ファイルパスを確認してください。")
        return

    # ★モバイル対応: 高さ110vhを60vh相当に修正し、ボタンが見えるように調整★
    html_content = f"""
    <div style="
        position: relative;
        width: 100%;
        height: 60vh; /* モバイルでボタンが隠れない高さ */
        border-radius: 12px;
        overflow: hidden;
...
if 'coord_season_key' not in st.session_state:
    st.session_state.coord_season_key = "Winter"
...
def show_diagnosis_page():
    ...
    try:
        ...
        with st.spinner("診断を実行中です..."):
            # ★外部モジュール 'color_analyzer.py' を呼び出し★
        ...
        # ここで 'name 'cite' is not defined' エラーが発生しているはずです
...
def show_result_page():
    ...
    if is_info_selected:
        ...
        if os.path.exists(image_path):
            # ★モバイル対応: width指定を削除し、画面幅に合わせる★
            st.image(image_path, caption=f"【{full_season_key}】に似合うイメージ")
        else:
